...
import { paymentAPI, orderAPI } from "../lib/config/api";
...
function Checkout() {
	...
	useEffect(() => {
		...
		if (location.state?.orderId) {
			...
			if (location.state?.amount) {
				...
				fetchOrderAmount(location.state.orderId);
			}
		} else {
			setError("Order ID missing. Please return to cart and try again.");
		}
	}, [location.state]);

	const fetchOrderAmount = async (orderId) => {
		...
		try {
			...
			if (response.data && response.data.total_price) {
				...
				setError(
					"Unable to retrieve order amount. Please return to cart.",
				);
			}
		} catch (err) {
	...
	const handlePay = async (e) => {
		...
		try {
			...
			const response = await paymentAPI.payWithMpesa(phone, orderId);
			...
			<div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-8 px-4">
				<div className="max-w-md mx-auto text-center">
					<div className="bg-white rounded-2xl shadow-xl p-8">
						<div className="animate-spin rounded-full h-12 w-12 border-b-2 border-momma-pink mx-auto mb-4"></div>
