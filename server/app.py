...
class Signup(Resource):
    def post(self):
        ...
        # ✅ FIX: Use string identity with additional claims
        ...
        create_token = create_access_token(identity=user_identity, additional_claims=additional_claims)
...
class Login(Resource):
    def post(self):
        ...
        if user and bcrypt.check_password_hash(user.password, password):
            ...
            # Store user data in a separate claim (not in sub)
            additional_claims = {
                'name': user.name,
                'email': user.email,
                'phone': user.phone,
